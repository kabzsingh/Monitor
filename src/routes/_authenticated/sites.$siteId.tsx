...
export const Route = createFileRoute("/_authenticated/sites/$siteId")({ component: SiteDetail });

interface Meter { id: string; meter_type: "wash"|"fresh_water"|"chemical"; name: string; unit: string; capacity: number|null; low_threshold: number|null; device_key: string }
interface Reading { meter_id: string; value: number; recorded_at: string }
...
function SiteDetail() {
  ...
  const load = async () => {
    const [{ data: s }, { data: m }] = await Promise.all([
      ...
      supabase.from("site_meters").select("id,meter_type,name,unit,capacity,low_threshold,device_key,position").eq("site_id", siteId).order("position"),
    ]);
    setSite(s as any);
    setMeters((m as any) ?? []);
    const since = new Date(Date.now() - 24 * 60 * 60_000).toISOString();
    const { data: r } = await supabase
      .from("readings").select("meter_id,value,recorded_at")
      .eq("site_id", siteId).gte("recorded_at", since)
  ...
  useEffect(() => {
    load();
    const ch = supabase.channel(`site-${siteId}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "readings", filter: `site_id=eq.${siteId}` }, () => load())
      .subscribe();
  ...
  const stats = useMemo(() => {
    ...
    let washToday = 0, washLifetime = 0, freshToday = 0, freshLifetime = 0;
    ...
    return { washToday, washLifetime, freshToday, freshLifetime, latestByMeter };
  }, [readings, meters]);
  ...
  const washChart = useMemo(() => {
    ...
    for (const r of readings) {
      ...
      const d = new Date(r.recorded_at); d.setMinutes(0,0,0);
      const k = d.toISOString();
      if (k in buckets) buckets[k] += Number(r.value);
    }
    return Object.entries(buckets).map(([k, v]) => ({ time: new Date(k).toLocaleTimeString([], { hour: "2-digit" }), washes: v }));
  }, [readings, washMeters]);
  ...
  const freshChart = useMemo(() => {
    ...
    return Object.entries(buckets).map(([k, v]) => ({ time: new Date(k).toLocaleTimeString([], { hour: "2-digit" }), liters: v }));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <StatCard icon={Activity} label="Wash lifetime" value={stats.washLifetime.toLocaleString()} sub="Last 24h shown" />
        ...
        <StatCard
          ...
          tone={chemicalMeters.some((m) => {
            ...
            return last && m.low_threshold !== null && Number(last.value) <= Number(m.low_threshold);
          }) ? "danger" : "success"}
            ...
            {chemicalMeters.map((m) => {
              ...
              return (
                <ChemicalGauge
                  ...
                  capacity={m.capacity} unit={m.unit}
...
function ChartCard({ title, data, dataKey, stroke }: { title: string; data: any[]; dataKey: string; stroke: string }) {
  return (
